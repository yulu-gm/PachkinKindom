...
export class BootScene extends Phaser.Scene{
  ...
  preload():void{for(const role of ROLES)for(const name of Object.keys(ANIMS)as(keyof typeof ANIMS)[])this.load.spritesheet(`${role}-${name}`,spriteUrl(role,name),{frameWidth:96,frameHeight:96})}
  create():void{
